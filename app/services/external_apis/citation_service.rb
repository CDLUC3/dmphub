  ...
  class CitationService < BaseService
    class << self
      ...
      def process_json(doi:, json:)
        ...
        date = json.fetch('published-print', json.fetch('date-parts', []))
        date = json.fetch('created', {}).fetch('date-parts', []) unless date.any?
        return doi unless date.any?

        authors = json['author'].map { |author| "#{author['family']}, #{author['given'][0]}." }.join(', ')
        link = "<a href=\"#{doi}\" target=\"_blank\">#{doi}</a>"

        "#{authors} (#{date[0]}). \"#{json['title']}\" [#{json['type'].capitalize}]. In #{json['publisher']}. #{link}"
      end
