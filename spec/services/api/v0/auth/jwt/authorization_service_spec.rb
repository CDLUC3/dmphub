...
RSpec.describe Api::V0::Auth::Jwt::AuthorizationService do
  before(:each) do
  ...
  context 'instance methods' do
    ...
    it '#call defers to the private #client method' do
      allow(@service).to receive(:client).and_return(true)
      @service.call
      expect(@service).to have_received(:client)
    end
  end
  ...
  context 'private methods' do
    ...
    describe '#client' do
      it 'returns the client if its already set (singleton)' do
        allow(ApiClient).to receive(:find_by).and_return(@client)
        @service.send(:client)
        expect(ApiClient).not_to have_received(:find_by)
      end
    ...
    describe '#decoded_auth_token' do
      ...
      it 'adds "token expired" to errors when a JWT has expired' do
        allow(Api::V0::Auth::Jwt::JsonWebToken).to receive(:decode).and_raise(JWT::ExpiredSignature)
